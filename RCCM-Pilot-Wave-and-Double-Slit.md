...
```mermaid
flowchart LR
    S["Source: wave + localized particle"] --> W["Wave reaches the barrier"]
    W --> A["Wave branch through slit A"]
    W --> B["Wave branch through slit B"]
    A --> O["Branches overlap: phase terrain"]
    B --> O
    S --> P["Particle takes one actual slit"]
    O --> G["Guidance-current channels"]
    P --> G
    G --> D["One localized detection dot"]
    D --> M["Many launches mark the interference distribution"]
```
